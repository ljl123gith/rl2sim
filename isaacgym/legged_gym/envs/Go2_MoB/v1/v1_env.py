...
from legged_gym.envs.Go2_MoB.v1.v1_config import V1FlatCfg
...
class V1RobotFlat1(LeggedRobot):
    ...
    def _get_gait_phase(self):
        ...
        # For Trot gait: two diagonal pairs alternate
        # Logical foot order: FL, FR, BL, BR
        phase = self._get_phase()
        stance_mask = torch.zeros((self.num_envs, 2), device=self.device)
        stance_mask[:, 0] = phase < 0.5   # First diagonal: FL + BR
        stance_mask[:, 1] = phase >= 0.5  # Second diagonal: FR + BL
        return stance_mask
    ...
    def _create_envs(self):
        ...
        # # Create feet indices with Isaac Gym's natural order
        # isaac_feet_indices = torch.zeros(len(feet_names), dtype=torch.long, device=self.device, requires_grad=False)
        # for i in range(len(feet_names)):
        #     isaac_feet_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.actor_handles[0], feet_names[i])
        
        # # Create mapping from Isaac Gym order to logical order
        # # Isaac Gym order: ['BL_foot', 'BR_foot', 'FL_foot', 'FR_foot'] = [0, 1, 2, 3]
        # # Desired logical order: ['FL_foot', 'FR_foot', 'BL_foot', 'BR_foot'] = [0, 1, 2, 3]
        # feet_name_to_logical_idx = {
        #     'FL_foot': 0,  # Front Left
        #     'FR_foot': 1,  # Front Right  
        #     'BL_foot': 2,  # Back Left (Rear Left)
        #     'BR_foot': 3   # Back Right (Rear Right)
        # }
        
        # # Create logical feet indices array
        ...
        #     logical_idx = feet_name_to_logical_idx[name]
        #     self.feet_indices[logical_idx] = isaac_feet_indices[i]
        
        # print(f"Logical feet order: FL(0)={self.feet_indices[0]}, FR(1)={self.feet_indices[1]}, BL(2)={self.feet_indices[2]}, BR(3)={self.feet_indices[3]}")
    ...
    #     """
    #     Calculates a smoother, continuous reward for trotting.
    #     Instead of a single binary check, it rewards each component of the trot gait separately.
    #     Logical order: ['FL_foot', 'FR_foot', 'BL_foot', 'BR_foot'] = [0, 1, 2, 3]
    #     """
    #     contact = self.contact_forces[:, self.feet_indices, 2] > 1.0

    #     stance_mask = self._get_gait_phase()
    #     # stance_mask[:,0] is for FL+BR, stance_mask[:,1] is for FR+BL

    #     # --- Calculate individual components of the trot reward ---

    #     # 1. Reward diagonal synchronization
    #     # Ideal: contact[0]==contact[3] and contact[1]==contact[2]
    #     diagonal1_sync = (1 - torch.abs(contact[:,0].float() - contact[:,3].float()))
    #     diagonal2_sync = (1 - torch.abs(contact[:,1].float() - contact[:,2].float()))
        
    #     # 2. Reward phase matching
    #     # Ideal: contact[0] == stance_mask[0], contact[1] == stance_mask[1], etc.
    #     phase_match_FL = (1 - torch.abs(contact[:,0].float() - stance_mask[:,0].float())) # FL matches diagonal 1 phase
    #     phase_match_FR = (1 - torch.abs(contact[:,1].float() - stance_mask[:,1].float())) # FR matches diagonal 2 phase
    #     phase_match_BL = (1 - torch.abs(contact[:,2].float() - stance_mask[:,1].float())) # BL matches diagonal 2 phase
    #     phase_match_BR = (1 - torch.abs(contact[:,3].float() - stance_mask[:,0].float())) # BR matches diagonal 1 phase

    #     # --- Combine rewards ---
    #     # We average the rewards from all components
    #     total_sync_reward = (diagonal1_sync + diagonal2_sync) / 2.0
    #     total_phase_match_reward = (phase_match_FL + phase_match_FR + phase_match_BL + phase_match_BR) / 4.0
        
    #     # The final reward is the product of synchronization and phase matching quality
    #     smooth_trot_reward = total_sync_reward * total_phase_match_reward
        
    #     self.trot = smooth_trot_reward.mean() # For logging purposes
        
    #     return smooth_trot_reward * (torch.norm(self.commands[:, :2], dim=1) > 0.2)
    ...
    def _reward_trot(self):
        ...
        # Check if diagonal pairs are in sync with each other
        diagonal1_sync = (contact[:,0] == contact[:,3])  # FL == RR
        diagonal2_sync = (contact[:,1] == contact[:,2])  # FR == RL
        
        # Check if diagonal pairs match their expected gait phase
        diagonal1_phase_match = (contact[:,0] == stance_mask[:,0]) & (contact[:,3] == stance_mask[:,0])  # FL and RR both match phase 0
        diagonal2_phase_match = (contact[:,1] == stance_mask[:,1]) & (contact[:,2] == stance_mask[:,1])  # FR and RL both match phase 1
        
        # All conditions must be true for proper trot gait
        TROT = diagonal1_sync & diagonal2_sync & diagonal1_phase_match & diagonal2_phase_match
        
        # Debug prints (uncomment for debugging)
        if hasattr(self, 'debug_trot_counter'):
            self.debug_trot_counter += 1
        else:
        ...
        return TROT.to(torch.float32) * speed_condition
    ...
    #     Logical foot order: ['FL_foot', 'FR_foot', 'BL_foot', 'BR_foot'] = [0, 1, 2, 3]
    #     """
    #     # Get the height of all four feet
    #     self.feet_height = self.rigid_state[:, self.feet_indices, 2] - 0.02

    #     # Get the swing signal for each DIAGONAL pair from the gait phase
    #     stance_mask = self._get_gait_phase()  # stance_mask[:,0] for FL+BR, stance_mask[:,1] for FR+BL
    #     swing_mask = 1 - stance_mask
    #     swing_diagonal_FL_BR = swing_mask[:, 1]
    #     swing_diagonal_FR_BL = swing_mask[:, 0]

    #     # Create a swing mask for each INDIVIDUAL foot based on the logical order
    #     foot_swing_mask = torch.stack([
    #         swing_diagonal_FL_BR,  # FL (foot 0) is in the FL+BR diagonal
    #         swing_diagonal_FR_BL,  # FR (foot 1) is in the FR+BL diagonal
    #         swing_diagonal_FR_BL,  # BL (foot 2) is in the FR+BL diagonal
    #         swing_diagonal_FL_BR   # BR (foot 3) is in the FL+BR diagonal
    #     ], dim=1)

    #     # Define the target height curve for swinging feet
    #     phase = self._get_phase()
    #     target_height = torch.abs(torch.sin(2 * torch.pi * phase)) * self.cfg.rewards.target_foot_height
        
    #     # Calculate the height error for all feet
    #     # Unsqueeze target_height to broadcast correctly with feet_height [num_envs, 4]
    #     height_error = torch.abs(self.feet_height - target_height.unsqueeze(1))
        
    #     # Calculate the reward, but only for the feet that are supposed to be swinging
    #     reward = torch.exp(-torch.sum(height_error * foot_swing_mask, dim=1) * 10)
