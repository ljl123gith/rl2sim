...
class V1FlatCfg( LeggedRobotCfg ):
    ...
    class init_state( LeggedRobotCfg.init_state ):
        ...
        default_joint_angles = { # = target angles [rad] when action = 0.0
            'FL1_hip_joint': 0,
            'FR1_hip_joint': 0,   # [rad] 'RL_hip_joint': 0.1,   # [rad]
            'RL1_hip_joint': 0 ,  # [rad]
            'RR1_hip_joint': 0,   # [rad]

            'FL2_thigh_joint': 0.7,     # [rad]S
            'FR2_thigh_joint': 0.7,   # [rad]
            'RL2_thigh_joint': 0.7,     # [rad]
            'RR2_thigh_joint': 0.7,   # [rad]

            'FL3_calf_joint': -0.7,   # [rad]
            'FR3_calf_joint': -0.7,    # [rad]
            'RL3_calf_joint': -0.7,  # [rad]
            'RR3_calf_joint': -0.7,    # [rad]
        }

    class control( LeggedRobotCfg.control ):
        # PD Drive parameters:
        stiffness = {'hip': 20, 'thigh': 20, 'calf': 20}  # [N*m/rad]
        damping = {'hip': 0.5, 'thigh': 0.5, 'calf': 0.5}     # [N*m*s/rad]
        # action scale: target angle = actionScale * action + defaultAngle
        action_scale = 0.25
        gait_frequency = 1.0 # Hz, 步态频率，可以调整
        # decimation: Number of control action updates @ sim DT per policy DT
        decimation = 4

    class asset(LeggedRobotCfg.asset):
        file = '{LEGGED_GYM_ROOT_DIR}/resources/robots/v1/urdf/dogVLAI.urdf'
        name = "v1"
        foot_name = "foot"
    ...
    class domain_rand:
        ...
        stiffness_multiplier_range = [0.9, 1.5]  
        damping_multiplier_range = [0.9, 1.5]    
   ...
   # range to contain the real joint armature 
    ...
    class rewards( LeggedRobotCfg.rewards ):
        class scales:
            termination = -0.0
            tracking_lin_vel = 4* 1. / (.005 * 4)
            tracking_ang_vel = 4* 1. / (.005 * 4)
            lin_vel_z = 0.2* 1. / (.005 * 4)
            ang_vel_xy = 0  #-0.02
            orientation =   0.2 * 1. / (.005 * 4)# 0.2
            torques = -0.0002#
            dof_acc = -2.5e-7#-7
            # collision = 0 #-1.
            action_rate = -0.01   #  -0.1  # -0.01
            # stand_still = 0  # -10. #-1
            base_height=0.2
            # trot=1.0
            # feet_clearance=1.0
            # default_hip_pos=0  #-1.0
            # default_pos=0   #-0.15
    ...
    class noise:
        add_noise = True
        noise_level = 0.4 # scales other values #0.8
        class noise_scales:
            dof_pos = 0.01
            dof_vel = 1.5
            lin_vel = 0.1
...
class V1FlatCfgPPO( LeggedRobotCfgPPO ):
    seed = 1
    ...
    class runner:
        ...
        experiment_name = 'v1_trot'
        run_name = ''
        # load and resume
        resume = False
        load_run = -1 # "Sep22_10-34-59_" # -1 = last run
